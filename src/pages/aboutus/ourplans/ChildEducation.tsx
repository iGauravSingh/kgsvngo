...
const PostList = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${baseUrl}/api/categories/2?fields=name&populate[posts][fields][0]=title&populate[posts][fields][1]=date`
        );
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/api/categories/2?fields=name&populate[posts][filters][date][$gte]=${startDate}&populate[posts][filters][date][$lte]=${endDate}&populate[posts][fields][0]=title&populate[posts][fields][1]=date`
      );
      setPostList(response.data.data.attributes.posts.data);
    } catch (error) {
