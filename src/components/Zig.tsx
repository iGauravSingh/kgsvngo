...
const items = [
  {
    ...
    title: "Education",
    qoutes: "Education is what remains after one has forgotten what one learnt in school - Albert Einstein",
  ...
  },
  {
    ...
    title: "Health",
    qoutes: "Healthy citizens are the greatest asset that a country can have - Winston S Churchill",
  ...
  },
  {
    ...
    title: "Enviroment",
    qoutes: "We do not inherit the world from our ancestors; we borrow it from our children - Unknown",
  ...
  },
  {
    ...
    qoutes: "Empowering women is the key to building a future that we want - Amartya Sen",
  ...
  },
  {
    ...
    image: "/zig/agri.jpg",
    title: "Agriculture",
    qoutes: "It's been proven that of all the interventions to reduce poverty, improving agricultural productivity is the best - Bill Gates",
  ...
  },
  {
    ...
    qoutes: "Strenghten the rural areas by providing opportunity and fulfilling aspirations. This will put a stop to meaningless migration to urban centres, allowing both to flourish - KGSV  ",
...
const Zig = () => {
  return (
    <div className=" w-screen pt-5 pb-7 font-popins ">
      <Container>
        <h2 className=" text-center text-5xl text-greenngo">What does KGSV do ?</h2>
      {items.map((item) => (
        <div className=" mt-10">
          <h2 className=" text-center font-bold text-4xl text-greenngo">{item.title}</h2>
        <div key={item.id} className=" relative flex flex-col lg:flex-row items-center lg:justify-between">
          
          {/* text on right  */}
          <div className=" hidden md:block absolute -mt-44 ">
          <p className="left-0 font-Sacramento text-4xl w-[500px] text-ngodark font-semibold">{item.qoutes}</p>
          </div>
          <motion.div
          variants={boxVariant2}
          initial='hidden'
          whileInView='show'
          className=" w-[500px] lg:mt-80 mt-5 bg-white/20 shadow-lg  px-4 py-4 rounded-xl">
